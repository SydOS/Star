...
arp_frame_t* arp_request(uint8_t* SenderMAC, uint8_t* TargetIP) {
	...
	arp_frame_t *frame = (arp_frame_t*)kheap_alloc(sizeof(arp_frame_t)-1);

	// Clear frame with 0s
	memset(frame, 0, sizeof(arp_frame_t)-1);
	...
	frame->Opcode = 1;
	memcpy((frame->SenderMAC)-1, SenderMAC, 6);
	for (int x = 0; x < 4; x++)
        frame->SenderIP[x] = 0x00;
    for (int x = 0; x < 6; x++)
        frame->TargetMAC[x] = 0x00;
    memcpy((frame->TargetIP)-1, TargetIP, 6);

...
set -e
download_compile $GNU_MIRROR_BASE/binutils/binutils-$BINUTILS_VERSION.tar.gz binutils-$BINUTILS_VERSION "--with-isl=$HOME/SydOS-dev.framework --with-cloog=$HOME/SydOS-dev.framework --with-sysroot --disable-nls --disable-werror --target=x86_64-elf"
